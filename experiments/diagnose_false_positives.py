...
from promptbreak.main import DEFAULT_GUARD_THRESHOLD, classify_prompt, heuristic_risk
...
def diagnose_from_sweep(
    sweep: dict[str, Any],
    dataset_path: Path,
    confidence_threshold: float = DEFAULT_GUARD_THRESHOLD,
) -> dict[str, Any]:
    """Build the same attribution report from an existing zero-error sweep."""
    if sweep.get("errors"):
        raise ValueError("Sweep contains classifier errors")
    dataset = load_dataset(dataset_path)
    sweep_dataset = sweep.get("dataset", {})
    if (
        sweep_dataset.get("name") != dataset.get("name")
        or sweep_dataset.get("version") != dataset.get("version")
    ):
        raise ValueError("Sweep and dataset identity do not match")
    source_rows = {row["case_id"]: row for row in sweep.get("rows", [])}
    rows = []
    for case in (case for case in dataset["cases"] if case["kind"] == "benign"):
        if case["id"] not in source_rows:
            raise ValueError(f"Sweep is missing benign case {case['id']}")
        source = source_rows[case["id"]]
        heuristic = source["heuristic"]
        llm = source["llm"]
        llm_blocked = bool(llm["malicious"]) and float(llm["confidence"]) >= confidence_threshold
        heuristic_blocked = bool(heuristic["blocked"])
        if heuristic_blocked and llm_blocked:
            attribution = "both"
        elif heuristic_blocked:
            attribution = "heuristics_only"
    ...
    return {
        ...
        "rows": rows,
        "derived_from": "guard threshold sweep; no additional model calls",
        "scope_note": (
            "This report attributes input-guard false positives only. Context and "
            "output layers are intentionally not re-run; end-to-end benign blocks "
            "caused by caught model leaks must be interpreted separately."
...
def main() -> None:
    ...
    parser.add_argument(
        "--sweep-report",
        type=Path,
        help="Reuse a zero-error threshold sweep instead of calling the model again.",
    )
    parser.add_argument(
        "--threshold",
        type=float,
    ...
    if args.sweep_report:
        report = diagnose_from_sweep(
            json.loads(args.sweep_report.read_text(encoding="utf-8")),
            args.dataset,
            args.threshold,
        )
    else:
